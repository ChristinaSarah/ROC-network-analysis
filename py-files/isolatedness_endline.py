...
friend_support_set = set(df["friend_1"].dropna()) \
                   | set(df["friend_2"].dropna()) \
                   | set(df["friend_3"].dropna())
support_support_set = set(df["support_1"].dropna()) \
                       | set(df["support_2"].dropna()) \
                       | set(df["support_3"].dropna())
...
# 1) isolated_friend_in: 1 if not listed by anyone else as emotional support, 0 otherwise
df["isolated_friend_in"] = df["student_id_str"].apply(lambda x: 0 if x in friend_support_set else 1)

# 2) isolated_support_in: 1 if not listed by anyone else as academic support, 0 otherwise
df["isolated_support_in"] = df["student_id_str"].apply(lambda x: 0 if x in support_support_set else 1)

# 3) isolated_friend_out: 1 if student does not list anyone as friend_1/friend_2/friend_3, 0 otherwise
df["isolated_friend_out"] = df[["friend_1", "friend_2", "friend_3"]]\
    .isnull()\
    .all(axis=1)\
    .astype(int)

# 4) isolated_support_out: 1 if student does not list anyone as support_1/support_2/support_3, 0 otherwise
...
df.to_csv("/workspaces/ROC-network-analysis/output-files/roc_isolatedness.csv", index=False)